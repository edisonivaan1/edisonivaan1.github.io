...
export const authService = {
  ...
  register: async (userData: {
    ...
    if (await isOfflineMode()) {
      return offlineAuthService.register(userData);
    }
  ...
  login: async (credentials: { email: string; password: string }) => {
    // Verificar si estamos en modo offline
    if (await isOfflineMode()) {
      return offlineAuthService.login(credentials);
    }
...
export const progressService = {
  ...
  getProgress: async () => {
    // Verificar si estamos en modo offline
    if (await isOfflineMode()) {
      return offlineProgressService.getProgress();
    }
  ...
  completeLevel: async (data: CompleteLevelData): Promise<ApiResponse> => {
    // Verificar si estamos en modo offline
    if (await isOfflineMode()) {
      return offlineProgressService.completeLevel(data);
    }
  ...
  getStats: async () => {
    // Verificar si estamos en modo offline
    if (await isOfflineMode()) {
      return offlineProgressService.getStats();
    }
...
export const achievementService = {
  ...
  getAllAchievements: async () => {
    // Verificar si estamos en modo offline
    if (await isOfflineMode()) {
      return offlineAchievementService.getAllAchievements();
    }
  ...
  getUserAchievements: async (includeAll: boolean = true) => {
    // Verificar si estamos en modo offline
    if (await isOfflineMode()) {
      return offlineAchievementService.getUserAchievements();
    }
